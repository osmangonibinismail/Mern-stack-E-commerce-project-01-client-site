...
const VerticalCartt = ({loading,data = []}) => {
    ...
    <div className='grid grid-cols-[repeat(auto-fit,minmax(260px,300px))]  md:justify-between justify-center md:gap-4 overflow-x-scroll scrollbar-none transition-all'>
            {
                loading ? (
                    ...
                    data.map((product, index) => {
                        return (
                            <Link to={"/product/" + product?._id} className='w-full min-w-[280px] md:min-w-[300px] max-w-[280px] md:max-w-[310px]  bg-white shadow rounded-sm ' onClick={scrollTop}>
                                ...
                                <div className='p-4 grid gap-3'>
                                    ...
                                    <button className='text-sm bg-lime-500 hover:bg-lime-600 text-white px-3 py-0.5 rounded-full' onClick={(e) => handleAddToCart(e, product?._id)}>Add to Cart</button>
                                </div>
                            </Link>
                        )
                    })
